...
// Adds a "Vercidium Audio" group with a material dropdown and a "Use Flat Transmission" checkbox to
// the Inspector for every Node3D, so geometry can be configured without hand-typing the
// "vercidium_audio_material"/"vercidium_audio_use_flat_transmission" metadata. Shown on all Node3D
// types (not just MeshInstance3D/CSGShape3D) because VAWorld::add_primitive applies a node's
// material and transmission override to every descendant that doesn't override it itself - setting
...
// edited scene; the transmission setting only takes effect on MeshInstance3D primitives (vaMeshPrimitiveSetUseFlatTransmission).
